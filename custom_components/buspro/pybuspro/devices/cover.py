...
class Cover(Device):
    def __init__(self, buspro, device_address, channel_number, name="", opening_time=20,
    ...
    async def set_position(self, position):
        ...
        self._state_changetime = self._opening_time + (
                (position / 100) * self._opening_time)  # will take 30 seconds to do 50% for 20s assumed time
        ...
        await asyncio.sleep(
            (position / 100) * self._opening_time)  # if 50% position then wait for 15 seconds if opening time is 30
    ...
    def current_cover_position(self):
        ...
        if self._status == STATE_OPENING:
            ...
            _LOGGER.debug(
                "Curtain Opening '{}' elapsed time is {} current position is {} time to change position i {}".format(
                    self._device_address, elapsed_time,
            ...
            if elapsed_time < self._state_changetime:
                ...
                _LOGGER.debug(
                    "Curtain Opening '{}' difference of change is {} completed diff % is {} and current position is {}".format(
                        self._device_address, difference,
